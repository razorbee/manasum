...
if (!function_exists('pathwell_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'pathwell_hovers_theme_setup3', 3 );
	function pathwell_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		pathwell_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'pathwell'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'pathwell') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'pathwell'),
					'slide_left'	=> esc_html__('Slide from Left',	'pathwell'),
					'slide_right'	=> esc_html__('Slide from Right',	'pathwell'),
					'slide_top'		=> esc_html__('Slide from Top',		'pathwell'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'pathwell'),
//					'arrow'			=> esc_html__('Arrow',				'pathwell'),
				),
				//"type" => "select"
				"type" => "hidden"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'pathwell'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'pathwell') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'pathwell'),
					'icon'	=> esc_html__('Icon',	'pathwell'),
					'zoom'	=> esc_html__('Zoom',	'pathwell'),
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('pathwell_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'pathwell_hovers_theme_setup9', 9 );
	function pathwell_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'pathwell_hovers_frontend_scripts', 1010 );
		add_filter( 'pathwell_filter_localize_script','pathwell_hovers_localize_script' );
		add_filter( 'pathwell_filter_merge_scripts',	'pathwell_hovers_merge_scripts' );
		add_filter( 'pathwell_filter_merge_styles',	'pathwell_hovers_merge_styles' );
		add_filter( 'pathwell_filter_merge_styles_responsive', 'pathwell_hovers_merge_styles_responsive' );
		add_filter( 'pathwell_filter_get_css', 		'pathwell_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'pathwell_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'pathwell_hovers_frontend_scripts', 1010 );
	function pathwell_hovers_frontend_scripts() {
		if ( pathwell_is_on(pathwell_get_theme_option('debug_mode')) && pathwell_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'pathwell-hovers', pathwell_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('pathwell_hovers_merge_styles')) {
	//Handler of the add_filter( 'pathwell_filter_merge_styles', 'pathwell_hovers_merge_styles' );
	function pathwell_hovers_merge_styles($list) {
...
if (!function_exists('pathwell_hovers_merge_styles_responsive')) {
	//Handler of the add_filter( 'pathwell_filter_merge_styles_responsive', 'pathwell_hovers_merge_styles_responsive' );
	function pathwell_hovers_merge_styles_responsive($list) {
...
if (!function_exists('pathwell_hovers_localize_script')) {
	//Handler of the add_filter( 'pathwell_filter_localize_script','pathwell_hovers_localize_script' );
	function pathwell_hovers_localize_script($arr) {
		$arr['button_hover'] = pathwell_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('pathwell_hovers_merge_scripts')) {
	//Handler of the add_filter( 'pathwell_filter_merge_scripts', 'pathwell_hovers_merge_scripts' );
	function pathwell_hovers_merge_scripts($list) {
...
if ( !function_exists('pathwell_hovers_add_icons') ) {
	function pathwell_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					pathwell_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'pathwell') : esc_html__('Buy now', 'pathwell')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'pathwell');
						else
							esc_html_e('Details', 'pathwell');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'pathwell_hovers_get_css' ) ) {
	//Handler of the add_filter( 'pathwell_filter_get_css', 'pathwell_hovers_get_css', 10, 4 );
	function pathwell_hovers_get_css($css, $colors, $fonts, $scheme='') {
